...
function DiffBadge({ diff }) {
  ...
  return (
    <span
      className={
        "text-xs px-2 py-0.5 rounded-full font-medium " + (colors[diff] || "")
      }
    >
      {diff}
    </span>
...
export default function QuestionsView({
  ...
  const [noteModal, setNoteModal] = useState(null); // { id, title, note }
  ...
  const filtered = QUESTIONS.filter((q) => {
    ...
    if (statusFilter === "Revision" && !progress.revision.includes(q.id))
      return false;
    return true;
  });
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-6">
          {TOPICS.map((t) => {
            ...
            const topicSolved = topicQs.filter(
              (q) => progress.solved[q.id],
            ).length;
            return (
              <button
                key={t.id}
                onClick={() =>
                  setTopicFilter(topicFilter === t.id ? "All" : t.id)
                }
                className={
              ...
              >
                ...
                <div className="text-xs font-medium text-gray-300 leading-tight truncate">
                  {t.name}
                </div>
                <div className="text-xs text-gray-500 mt-0.5">
                  {topicSolved}/{topicQs.length}
                </div>
              </button>
            );
          })}
              ...
              {filtered.map((q) => {
                ...
                return (
                  <div
                    key={q.id}
                    className={
                      "flex items-center gap-3 px-4 py-3 hover:bg-[#1C2128] transition-colors " +
                      (isSolved ? "opacity-60" : "")
                    }
                  >
                    <button
                    ...
                    <button
                      ...
                      className={
                        "flex-shrink-0 text-lg leading-none transition-colors " +
                        (isRevision
                          ? "text-yellow-400"
        ...
        >
          ...
          >
            ...
            <div className="flex justify-end gap-2 mt-3">
              ...
              <button
                onClick={async () => {
                  await onSaveNote(noteModal.id, noteModal.note);
                  setNoteModal(null);
                }}
